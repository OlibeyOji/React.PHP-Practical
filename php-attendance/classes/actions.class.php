...
class Actions{
    ...
    public function save_class(){
        ...
        if($check->num_rows > 0){
            return ['status' => 'error', 'msg' => 'Office Name Already Exists!'];
        }else{
            $qry = $this->conn->query($sql);
            if($qry){
                if(empty($id)){
                    $_SESSION['flashdata'] = [ 'type' => 'success', 'msg' => "New Office has been added successfully!" ];
                }else{
                    $_SESSION['flashdata'] = [ 'type' => 'success', 'msg' => "Office Data has been updated successfully!" ];
                }
